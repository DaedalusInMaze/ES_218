...
ggplot(dat_age, aes(year, med, col = Region)) + geom_point() +
  ...
  ggtitle('Median age of accident drivers') +
  theme(
    plot.title = element_text(color="black", size=11, face="bold.italic"),
    axis.title.x = element_text(color="black", size=11, face="bold"),
    axis.title.y = element_text(color="black", size=11, face="bold"))
...
Thus, we see that there is a regional difference in severity when I apply the single linear regression. However, it is important to check if the result hold by looking at the residuals plot.
...
I again checked the residuals to see if the conditiosn for the linear model holds.
